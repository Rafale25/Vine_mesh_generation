...
void main() {
    ...
    float noise_value = map(snoise(v_position*2.0), 0.0, 1.0, 0.2, 1.0);
    vec3 c = color1 + (color2 * noise_value);

    // out_branch_color = vec4(v_branch_color, 1.0);
    out_branch_color = vec4(unpackColor(v_branch_color), 1.0);
